...
def  sql_delete(data):   #dormdb_way表插入一条数据DELETE FROM table_name
        conn = sqlite3.connect('db.sqlite3')
        cursor = conn.cursor()
        sql ="DELETE FROM dormdb_Way WHERE devID= %s "% data.devID
        cursor.execute(sql)
        cursor.close()
        conn.commit()
        print (u'sql_delete ## ',data.devID)
        conn.close()
def  sql_insert(data):   #dormdb_way表插入一条数据
        conn = sqlite3.connect('db.sqlite3')
        cursor = conn.cursor()
        cursor.execute("insert into dormdb_Way (devID,\
           relay,time)\
        ...
        cursor.close()
        conn.commit()
        print (u'sql_insert ## ',data.devID)
        conn.close()


          

def  sql_update(data):   #更新dormdb_way表一条数据
        conn = sqlite3.connect('db.sqlite3')
        cursor = conn.cursor()
        sql = " UPDATE dormdb_Way  SET relay = '%s'   WHERE devID= %s " % (data.relay,data.devID)
        cursor.execute(sql)
        cursor.close()
        conn.commit
        conn.close()
	print (u'sql_update ##  ','id= ',data.devID,'##  ',data.relay)


def sql_search(data):    #搜索dormdb_way表
        conn = sqlite3.connect('db.sqlite3')
        cursor = conn.cursor()
        sql = "SELECT * FROM dormdb_way \
               WHERE devID = '%s'" % (data.devID)
        cursor.execute(sql)
        row = cursor.fetchall()
        print row
#        devID = row[1]
#        way = row[2]
        cursor.close()
#        print ('sql-search ID#',devID,'##way  ',way)
        return row
def update_check(data):
    way = data[7:12]
    devID = data[1:3]
    if (sql_search(rec) ==('kid','ing')):
        sql_insert(rec)
        print ('插入way 数据')
        return  False
  #没有找到，插入一条
